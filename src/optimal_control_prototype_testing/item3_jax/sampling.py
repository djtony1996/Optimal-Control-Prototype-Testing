...
def clip_controls(u: Array, problem: TrivialLQRProblem) -> Array:
    lower = jnp.asarray(problem.u_min, dtype=u.dtype)
    upper = jnp.asarray(problem.u_max, dtype=u.dtype)
    return jnp.clip(u, lower, upper)


def stage_cost(problem: TrivialLQRProblem, x: Array, u: Array) -> Array:
...
def rollout_controls(problem: TrivialLQRProblem, controls: Array) -> tuple[Array, Array]:
    ...
    running_cost = jax.vmap(lambda xk, uk: stage_cost(problem, xk, uk))(states[:-1], controls).sum()
    total_cost = running_cost + terminal_cost(problem, states[-1])
    return states, total_cost


def sample_rollout_costs(
    problem: TrivialLQRProblem,
    nominal_controls: Array,
    noise: Array,
) -> tuple[Array, Array]:
    trial_controls = clip_controls(nominal_controls[None, :, :] + noise, problem)
    states, costs = jax.vmap(lambda controls: rollout_controls(problem, controls))(trial_controls)
    return states, costs


def make_initial_controls(problem: TrivialLQRProblem) -> Array:
...
def solve_trivial_lqr_with_mppi(
    ...
    def body(carry: tuple[Array, Array], _i: Array) -> tuple[tuple[Array, Array], None]:
        ...
        _, costs = sample_rollout_costs(problem, controls, noise)
    ...
    states_star, cost_star = rollout_controls(problem, controls_star)
...
def solve_trivial_lqr_with_cem(
    ...
    def body(carry: tuple[Array, Array, Array], _i: Array) -> tuple[tuple[Array, Array, Array], None]:
        ...
        _, costs = jax.vmap(lambda controls: rollout_controls(problem, controls))(trial_controls)
