...
```bash
PYTHONPATH=src .venv/bin/python -m optimal_control_prototype_testing.item1_jax.run_item1 --problem nonlinear --constraint-mode hard --dt 0.05
```

To run with a longer total time (useful when the solver needs more time to reach the goal):

```bash
PYTHONPATH=src .venv/bin/python -m optimal_control_prototype_testing.item1_jax.run_item1 --problem nonlinear --constraint-mode hard --final-time 8.0
```

Both flags can be combined:

```bash
PYTHONPATH=src .venv/bin/python -m optimal_control_prototype_testing.item1_jax.run_item1 --problem nonlinear --constraint-mode hard --dt 0.05 --final-time 6.0
```
...
```python
!PYTHONPATH=src python -m optimal_control_prototype_testing.item1_jax.run_item1 --problem nonlinear --constraint-mode hard --dt 0.05
```

To run with a longer total time:

```python
!PYTHONPATH=src python -m optimal_control_prototype_testing.item1_jax.run_item1 --problem nonlinear --constraint-mode hard --final-time 8.0
```
